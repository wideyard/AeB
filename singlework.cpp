...
singlework::singlework(QWidget *parent) :
    QWidget(parent),
    ui(new Ui::singlework)
{
    ui->setupUi(this);
...
void singlework::initGame()
{
    ...
    for(i=0;i<BUILDSIZE;i++)                        //摆放岩石
    {
        sthExist[build[i][0]][build[i][1]]=7;
    }
    for(i=0;i<PLANTSIZE;i++)                                //摆放植物
    {
        int x,y;
        do
        {
    ...
    for(i=0;i<ENEMYSIZE;i++)
    {
        ...
        while(sthExist[x][y]!=0);
        normalEnemy[i]=new Enemy(x,y,0,700/FRAME);
        sthExist[x][y]=4;
    }
    ...
    for(i=0;i<BOSSSIZE;i++)
    {
        ...
        while(sthExist[x][y]!=0);
        boss[i]=new Enemy(x,y,1,700/FRAME);
        sthExist[x][y]=4;
    }
}

void singlework::initWidget(void)
{
    lifeLabel=new QLabel(this);
    lifeLabel->setText("1P HP:");
    lifeLabel->setGeometry(10,5,50,30);

    lifeNum=new QLCDNumber(this);
    lifeNum->setSegmentStyle(QLCDNumber::Flat);
    lifeNum->setGeometry(65,5,120,30);
    ...
    mdsingle=new QSound(":/sound/lone brave.wav");
}

void singlework::initconnect(void)
{
    connect(timer,SIGNAL(timeout()),this,SLOT(process()));
    connect(startButton,SIGNAL(clicked()),this,SLOT(gameStart()));
}
void singlework::gameStart(void)
{
    lifeNum->display(player->restLife);
    if(!timer->isActive())
        timer->start(FRAME);
    mdsingle->play();
    update();
}
void singlework::process(void)
{
    playerMove();
    bombAbout();
    itemAbout();
...
void singlework::keyPressEvent(QKeyEvent * event)
{
    if(timer->isActive())
    {
        switch(event->key())
...
void singlework::layBomb()
{
        if((player->bombCount) < (player->bombCountLimit))
        {
            mdlay->play();
            Bomb *p=head;
            while(p->next!=0)
            ...
            p->next=new Bomb(player->x,player->y);
            player->bombCount=player->bombCount+1;
            sthExist[player->x][player->y]=6;
        }
}

void singlework::gameOver(int res)
{
    if(res==0)
    {
...
void singlework::paintEvent(QPaintEvent *)
{
    ...
    QString bossStr=":/images/virus.png";
    ...
    for(int i=0;i<PLANTSIZE;i++)
    {
        if(sthExist[plant[i]->x][plant[i]->y]==3)
        {
            painter.drawImage(10+BLOCK*(plant[i]->x),40+BLOCK*(plant[i]->y),plantImg);
                                              ...
                                              //载人
    QImage image;
    QString imgStr[4]={":/images/doc1.jpg",":/images/doc2.jpg",":/images/doc3.jpg",":/images/doc4.jpg"};
    int x=10+BLOCK*(player->x);
    int y=40+BLOCK*(player->y);

    if(player->direction > 0)
    {
        player->defDir=player->direction-1;
        image.load(imgStr[player->defDir]);
...
void singlework::playerMove(void)
{
    switch(player->direction)
    {
        case 1:   //左
            if(player->x > 0)
            {
                if(sthExist[(player->x)-1][player->y]<=5)
                {
                    if(sthExist[(player->x)-1][player->y]==0)
                    {
                        sthExist[(player->x)-1][player->y]=5;
                    }
                    if(sthExist[player->x][player->y]==5)
                    {
                        sthExist[player->x][player->y]=0;
                    }
                    player->x=player->x-1;
                }
            }
        ...
        case 2:     //下
            if(player->y < 14)
            {
                ...
                if(sthExist[player->x][(player->y)+1]<=5)
                {
                    if(sthExist[player->x][player->y+1]==0)
                    {
                        sthExist[player->x][player->y+1]=5;
                    }
                    if(sthExist[player->x][player->y]==5)
                    {
                        sthExist[player->x][player->y]=0;
        ...
        case 4:       //上
            ...
            break;
        default:
            if((sthExist[player->x][player->y]==0))
            {
                sthExist[player->x][player->y]=5;
            }
            player->direction=0;
            break;
    }
...
void singlework::bombAbout(void)
{
    Bomb *p=head->next;
    while(p)
    {
        ...
        if(tmp < (1000/FRAME))
        {
            ...
            if(tmp < (900/FRAME))
            {
                ...
                if(tmp < (800/FRAME))
                {
                    ...
                    if(((p->y)<13)&&(sthExist[p->x][p->y+2]<7))
                        sthExist[p->x][(p->y)+2]=2;
                    if(((player->atackRange) > 2)&&(tmp < (700/FRAME)))
                    {
                        if(((p->x)>2)&&(sthExist[p->x-3][p->y]<7))
...
void singlework::judgement(void)
{
    if(sthExist[player->x][player->y]==1)
    {
        switch(item->id)
        {
        case ID_COUNT:
            player->bombCountLimit=player->bombCountLimit+1;
            break;
        case ID_POWER:
            player->atackRange=player->atackRange+1;
            break;
        case ID_HEAL:
            player->restLife=player->restLife+5;
            lifeNum->display(player->restLife);
            break;
        default:
            break;
    ...
    else if(sthExist[player->x][player->y]==2)
    {
        player->restLife=(player->restLife)-1;
        lifeNum->display(player->restLife);
    }
    else if(sthExist[player->x][player->y]==3)
    {
        player->restLife=(player->restLife)-1;
        lifeNum->display(player->restLife);
    }
    else if(sthExist[player->x][player->y]==4)
    {
        player->restLife=(player->restLife)-1;
        lifeNum->display(player->restLife);

    }
    if((player->restLife)<=0)
        gameOver(0);
    else
...
void singlework::enemyMove(Enemy *enemy)
{
    ...
    if(enemy->moveTime==0)
    {
        ...
        int x,y;
        x=abs((player->x)-(enemy->x));
        y=abs((player->y)-(enemy->y));

        if(x>y)                               //横坐标更长，横向靠近
        {
           if((player->x)<(enemy->x))           //玩家1在enemy左边
           {
                ...
                {
                    if((player->y)<(enemy->y)) //如果玩家1在enemy上面
                    {
        ...
        {
           if((player->y)<(enemy->y))           //玩家1在enemy上边
           {
               if(sthExist[enemy->x][enemy->y-1]<6)//如果enemy上边没石头，炸弹,则向左移动
               {
                ...
                {
                    if((player->x)<(enemy->x)) //如果玩家1在enemy左面
                    {
                        if(sthExist[enemy->x-1][enemy->y]<6)//如果enemy左边没石头，炸弹,则向左移动
                        {
                            if((sthExist[enemy->x-1][enemy->y]==0)||(sthExist[enemy->x-1][enemy->y]==5))
